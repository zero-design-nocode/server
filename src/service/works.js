...
async function createWorkService(data = {}, content = {}) {
    ...
    // MongoDB 数据库通过符合 WorkContentModel 规定格式的数据创建作品内容
    // MongoDB 数据库对新建的数据会默认给一个随机的 _id 字段
    // 其会包含在创建函数调用的返回值内
    // 这里可以将其获取出来，保存进 MySQL 数据库的 contentId 字段
    // 这样就建立了 MySQL 数据库中作品表和 MongoDB 数据库中作品内容数据之间的关联
...
async function updateWorkService(data = {}, whereOpt = {}) {
    ...
    if (_.isEmpty(updateData)) {
        // 更新数据为空
        // 这也可能正常，例如用户只更新 content
        // content 是存储到 MongoDB 的，不会更新 MySQL
        return true
    }
